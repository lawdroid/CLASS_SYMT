...
1. **β is not constrained by the data.** The 3b marginal is flat across the
   full prior (0, 2.4]; the 3c marginal mirrors its narrow B1 prior. In both
   cases the posterior shape equals the prior shape — the data has no opinion
   on β across either window. The data-resolved coupling is the ratio
   g ≡ β/c_D, bounded at **g < 0.20 (95%)** from the 3b chain.
...
3. **H₀, Ωₘ, σ₈, and w_χ are robust across branches.** w_χ is tight at
...
![1D marginals for β, c_D, H0, Omega_m, sigma_8, w_chi overlaid for all three branches](analysis_2026-05-27/cross_branch_1d_marginals.png)
...
(each branch has a different β prior); c_D, H₀, w_χ overlay cleanly; Ωₘ and
...
- w_χ ≈ −0.99 across all three branches — effectively a cosmological constant
  in the data.
...
- "Data prefers β = X" — no β peak in any branch.
- "Evidence for or against non-zero coupling" — likelihood is flat in β.
- "Bimodality in 3b" — the KDE shows a single ridge; indirect signals can be
  misleading (see the [geometry lesson](phase3_geometry_lesson.html)).
